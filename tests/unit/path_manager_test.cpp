...
TEST_F(PathManagerTest, PlatformDetection) {
    ...
    bool platform_detected = PathManager::isLinux() || PathManager::isMacOS() || PathManager::isWindows();
    EXPECT_TRUE(platform_detected);

    // Only one platform should be detected
    int platform_count = 0;
    if (PathManager::isLinux())
        platform_count++;
    if (PathManager::isMacOS())
        platform_count++;
...
TEST_F(PathManagerTest, GetFileMappingsPath) {
    ...
    EXPECT_EQ(file_mappings_path.substr(file_mappings_path.length() - expected_suffix.length()), expected_suffix);

    // Should be different from data directory
    std::string data_dir = PathManager::getDataDirectory();
    EXPECT_NE(file_mappings_path.substr(0, file_mappings_path.find_last_of('/')), data_dir);
}