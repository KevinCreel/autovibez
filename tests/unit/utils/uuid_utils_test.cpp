...
TEST(HashIdUtilsTest, GenerateIdFromUrlEdgeCases) {
    ...
    EXPECT_TRUE(std::regex_match(emptyId, std::regex(R"([0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12})")));
    
    // Should handle special characters
    auto specialId = AutoVibez::Utils::HashIdUtils::generateIdFromUrl(specialChars);
    EXPECT_EQ(specialId.length(), 36);
    EXPECT_TRUE(std::regex_match(specialId, std::regex(R"([0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12})")));
    
    // Should handle long URLs
    auto longId = AutoVibez::Utils::HashIdUtils::generateIdFromUrl(longUrl);
    EXPECT_EQ(longId.length(), 36);
    EXPECT_TRUE(std::regex_match(longId, std::regex(R"([0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12})")));
