...
protected:
    ...
    void simulateThreadStartup() {
        ...
        MockThreading::state.active_threads = {
            "audio_callback", "render_thread", "ui_thread"
        };
...
TEST_F(ThreadingConcurrencyTest, UIThreadResponsivenessWorkflow) {
    ...
    EXPECT_TRUE(std::find(MockThreading::state.active_threads.begin(),
                         MockThreading::state.active_threads.end(),
...
TEST_F(ThreadingConcurrencyTest, ThreadSynchronizationStressWorkflow) {
    ...
    for (int i = 0; i < 100; ++i) {
        simulateAudioCallback();
        if (i % 10 == 0) simulateRenderThread();
        if (i % 20 == 0) simulateUIThread();
        if (i % 30 == 0) simulateDatabaseOperation();
    }
