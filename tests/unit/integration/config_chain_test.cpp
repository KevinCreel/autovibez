...
namespace MockConfigChain {
    ...
    std::vector<std::string> discovered_mix_files = {
        "mix1.mp3", "mix2.mp3", "mix3.mp3", "mix4.mp3"
    };
}
...
protected:
    void SetUp() override {
        ...
        MockConfigChain::discovered_mix_files = {
            "mix1.mp3", "mix2.mp3", "mix3.mp3", "mix4.mp3"
        };
    }
    ...
    bool simulatePathManagerInitialization() {
        ...
        MockConfigChain::state.paths_resolved = 
            MockConfigChain::state.config_directory_found && 
            MockConfigChain::state.data_directory_found;
    ...
    bool simulateConfigManagerInitialization() {
        ...
        if (MockConfigChain::filesystem.config_file_exists && 
            MockConfigChain::filesystem.config_file_readable) {
    ...
    void simulateFullChainInitialization() {
        ...
        MockConfigChain::state.full_chain_initialized = 
            path_success && config_success && mix_success;
    ...
    void simulateErrorPropagation(const std::string& failing_component) {
        ...
        MockConfigChain::state.error_propagated_upstream = 
            !MockConfigChain::state.full_chain_initialized;
    }
};
...
TEST_F(ConfigChainTest, MixDiscoveryIntegrationWorkflow) {
    ...
    MockConfigChain::discovered_mix_files = {
        "new_mix1.mp3", "new_mix2.mp3", "new_mix3.mp3"
    };
