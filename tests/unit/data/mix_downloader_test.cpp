...
TEST_F(MixDownloaderTest, GetLocalPathWithOriginalFilename) {
    ...
    AutoVibez::Data::Mix mix =
        createMockMix("test_id", "http://invalid-domain-that-doesnt-exist.com/mix.mp3", "original_name.mp3");
    std::string local_path = downloader.getLocalPathWithOriginalFilename(mix);
...
TEST_F(MixDownloaderTest, UnicodeFilePath) {
    ...
    AutoVibez::Data::Mix mix =
        createMockMix("test_id", "http://invalid-domain-that-doesnt-exist.com/mix.mp3", "sóng_ñáme.mp3");
    std::string local_path = downloader.getLocalPathWithOriginalFilename(mix);
...
TEST_F(MixDownloaderTest, ValidUrlFormatValidation) {
    ...
    for (const auto& url : valid_urls) {
        // Only test URL validation, not actual downloads
        EXPECT_TRUE(AutoVibez::Utils::UrlUtils::isValidUrl(url)) << "URL should be valid: " << url;
    }
}
