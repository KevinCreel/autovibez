...
protected:
    void SetUp() override {
        ...
        MockPersistence::database_records = {{"mix1", "Test Mix 1", true, 5, 120},
    ...
    void simulateApplicationShutdown() {
        ...
        if (MockPersistence::database_directory_exists && !MockPersistence::state.database_corrupted) {
            ...
            MockPersistence::state.favorites_count =
                std::count_if(MockPersistence::database_records.begin(), MockPersistence::database_records.end(),
    ...
    void simulateMixStateChange(const std::string& mix_id, int position, bool favorite) {
        ...
        auto it = std::find_if(MockPersistence::database_records.begin(), MockPersistence::database_records.end(),
                               [&mix_id](const MockPersistence::MockMixRecord& record) { return record.id == mix_id; });
...
TEST_F(DataPersistenceTest, MixStatePersistenceWorkflow) {
    ...
    auto mix1 = std::find_if(MockPersistence::database_records.begin(), MockPersistence::database_records.end(),
