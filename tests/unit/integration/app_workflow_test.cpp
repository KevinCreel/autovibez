...
protected:
    void SetUp() override {
        ...
        MockWorkflow::available_mixes = {{"mock_mix_1", "Test Mix 1", "Artist 1", "file:///mock/mix1.mp3", true},
    ...
    void simulateMixPlayback(const std::string& mix_id) {
        // Find mock mix
        auto it = std::find_if(MockWorkflow::available_mixes.begin(), MockWorkflow::available_mixes.end(),
                               [&mix_id](const MockWorkflow::MockMix& mix) { return mix.id == mix_id; });
